...
class UserController {
    static userRegistration = async(req, res)=>{
        ...
        if(user)
        {
            ...
            if(name && email && password && password_confirmation && tc)
            {
                ...
                {
                    res.send({"status":"failed", "message":"Password and Confirm Password are different"});
                }
            }
            else
            {
    ...
    static changePassword = async(req, res)=>{
        ...
        if(password && password_confirmation)
        {
            if(password !== password_confirmation)
            {
                res.send({"status":"failed", "message":"Confirm Password or Password is incorrect"});
            }
            else
            {
                const salt = await bcrypt.genSalt(10)
                ...
                res.send({"status":"success", "message":"Password saved Successfully"});
            }
        }
        else
        {
    ...
    static sendUserPasswordResetEmail = async(req, res)=>{
        ...
        if(email)
        {
            ...
            if(user)
            {
                ...
                res.send({"status":"success", "message":"Password reset link is sent on Email", "info": info});
            }
            else{
                res.send({"status":"failed", "message":"Email is not Exist"});
            }
    ...
    static userPasswordReset = async(req, res) => {
        ...
        {
            res.send({"status":"failed", "message": "Password link gor expired"});
        }
    }
}
